...
class SamMergeHead(MMDET_Mask2FormerHead):
    ...
    def forward(self, x: List[Tensor],
        ...
        for i in range(batch_size):
            # 将tensor从(1,3,640,640)转换为(3,640,640)
            image_temp = images[i]
            # 将tensor乘以标准差并加上平均值，还原到0-255的范围
            image_temp = image_temp * std[:, None, None] + mean[:, None, None]
            # 将tensor转换为uint8类型
            image_temp = image_temp.to(torch.uint8)
            image_temp = image_temp[[2, 1, 0], :, :]
            image_show = image_temp.permute(1, 2, 0).cpu().numpy()
            # 翻转
            # image_show = np.flip(image_show, axis=1)
            mask = mask_generator.generate(image_show)
            mask = sorted(mask, key=(lambda x: x['area']), reverse=True)
            mask = [item["segmentation"] for item in mask]
            # 将mask添加到masks列表中
            masks.append(mask)

        # mask第二步处理，对齐到num_queries
        mask_queries = []
        for i in range(batch_size):
            sam_masks = masks[i]
            if len(sam_masks) != 0:
                sam_masks = np.stack(sam_masks, axis=-1)
                sam_masks = torch.tensor(sam_masks).permute(2, 0, 1).to(device)
            else:
                h = images.shape[2]
                w = images.shape[3]
                sam_masks = torch.randn(1, h, w,
                                        device=device,
                                        dtype=torch.float32) / 6. + 0.5  # 3sigma = 1/2 --> sigma: 1/6
                sam_masks = float_to_mask(sam_masks, 0.5)
            num_gt = sam_masks.shape[0]
            h = sam_masks.shape[1]
            w = sam_masks.shape[2]
            if num_gt < self.num_queries:
                # 生成一些随机掩码，以使总的掩码数量达到 self.num_queries。
                # 这些随机掩码被创建为具有均值为0.5和标准差为1/6的随机浮点数，
                # 并然后使用 float_to_mask 函数将它们阈值化为二进制掩码，并与原始掩码 sam_masks 连接在一起，形成 x_start。
            ...
            # No noise version
            temp = x_start  # Use x_start directly

            temp = torch.clamp(temp, min=-1 * self.scale, max=self.scale)
            temp = ((temp / self.scale) + 1) / 2.
            temp = temp.to(torch.float32)
            mask_queries.append(temp)
