...
class SARFormerYolo(MMDET_Mask2FormerHead):
    ...
    def forward(self, x: List[Tensor],
        ...
        def show_gts(batch_data_samples,image,output_path='output_gt_image.png'):
            self.seg_local_visualizer.add_datasample('out_file_gt', image,
                                                batch_data_samples[0], show=False)
        ...
        results = model2.predict(images_re,verbose=False)

        try:
            mask_queries = []
            for i in range(batch_size):
                yolo_masks=results[i].masks.data.bool()
